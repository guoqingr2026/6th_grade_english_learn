...
const LAN_SYNC_DEBOUNCE_MS = 2500;
const LAN_SYNC_POLL_MS = 15000;
...
function updateGrammarUnitFilter() {
...
function updatePhraseUnitFilter() {
...
function updateVerbUnitFilter() {
  const unitValue = elements.verbUnitFilter?.value || "all";
  filteredVerbTriples = applyUnitFilter(verbTriples, unitValue);
  verbIndex = 0;
  renderVerb();
}
...
function renderVerb() {
  ...
  const verb = filteredVerbTriples[verbIndex];
  const unitLabel = verb.unit ? unitMeta[verb.unit] || `Unit ${verb.unit}` : "附录";
  elements.verbBase.textContent = `动词学习 (${unitLabel}): ${verb.base}`;
...
function bindEvents() {
  ...
  if (elements.grammarUnitFilter) elements.grammarUnitFilter.addEventListener("change", updateGrammarUnitFilter);
  if (elements.phraseUnitFilter) elements.phraseUnitFilter.addEventListener("change", updatePhraseUnitFilter);
  if (elements.verbUnitFilter) elements.verbUnitFilter.addEventListener("change", updateVerbUnitFilter);
