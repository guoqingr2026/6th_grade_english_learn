...
def find_unit_sources() -> list[tuple[int, Path, str]]:
    """Return (unit, path, kind) where kind is csv | txt | docx. Prefer csv > txt > docx."""
    found: dict[int, tuple[Path, str]] = {}
    priority = {"csv": 3, "txt": 2, "docx": 1}
    ...
    def consider(path: Path, kind: str) -> None:
        ...
        if not m:
            return
        u = int(m.group(1))
        if u not in found or priority[kind] > priority[found[u][1]]:
            found[u] = (path, kind)

    for d in WORD_DIRS:
        if not d.exists():
            continue
        for p in d.glob("Unit*"):
            low = p.name.lower()
            if low.endswith(".csv"):
                consider(p, "csv")
                ...
                consider(p, "docx")
    return sorted((u, p, k) for u, (p, k) in found.items())
...
def read_csv(path: Path) -> list[str]:
    ...
    page_key = fields.get("page") or fields.get("页码") or fields.get("pages")
    en_key = fields.get("en") or fields.get("英文") or fields.get("english")
    zh_key = fields.get("zh") or fields.get("中文") or fields.get("chinese")
    if not page_key:
        return lines
    last_page: int | None = None
    for row in reader:
        pg_raw = (row.get(page_key) or "").strip()
        ...
        if pg != last_page:
            ...
            last_page = pg
        en = (row.get(en_key) or "").strip() if en_key else ""
        zh = (row.get(zh_key) or "").strip() if zh_key else ""
        if en:
...
def resolve_page_ref(unit_num: int, page_ref: int, page_map: dict) -> tuple[int | None, int | None]:
    """Accept either textbook page (2–13) or PNG index (9, 10, …). Returns (png, book)."""
    seq = unit_png_sequence(unit_num, page_map)
    if not seq:
...
def main() -> None:
    ...
    if not sources:
        print("[WARN] No study files in word-sources/")
        print("       Supported: Unit1_*.docx | Unit1_*.csv | Unit1_*.txt")
        if OUT.exists():
