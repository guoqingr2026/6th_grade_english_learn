@echo off
rem Launch from project root (parent of release/) so data/scripts stay in sync
set "PROJ=%~dp0.."
cd /d "%PROJ%"
set PORT=8080
set PY_CMD=
...
if defined PY_CMD (
  ...
  "%PY_CMD%" "%PROJ%\scripts\import-question-cards.py"
  if errorlevel 1 goto :error

  echo [2/4] Sync question banks...
  "%PY_CMD%" "%PROJ%\scripts\sync-question-banks.py"
  if errorlevel 1 goto :error

  echo [3/4] CSV import + build study-hub...
  "%PY_CMD%" "%PROJ%\scripts\import-word-study.py"
  "%PY_CMD%" "%PROJ%\scripts\import-csv-to-library.py"
  "%PY_CMD%" "%PROJ%\scripts\build-study-hub.py"
  if errorlevel 1 goto :error
  "%PY_CMD%" "%PROJ%\scripts\verify-textbook-images.py"
...
"%PY_CMD%" "%PROJ%\scripts\lan_server.py" %PORT%
goto :eof
